...
def is_valid_username(username):
    ...
    if not r:
        ...
        raise ValidationError("Username format is not valid")

class AuthForm(FlaskForm):
    # shared form that groups most of our validations together to reduce repetition
    username = StringField("username", validators=[DataRequired(), Length(2, 30)])
    email = EmailField("email", validators=[DataRequired(), Email()])
    password = PasswordField("password", validators=[DataRequired(), EqualTo('confirm', message='Passwords must match'), Length(2, 30)])
    confirm = PasswordField("confirm", validators=[DataRequired(),  EqualTo('password', message='Passwords must match'), Length(2, 30)])
